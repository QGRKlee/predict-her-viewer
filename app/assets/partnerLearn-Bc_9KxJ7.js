import{g as x,a as w}from"./index-mQQMCGtp.js";import{a as g,P as u,b as B,c as b}from"./partnerContent-wuGPuOLt.js";import"./astro-CKYIcK6G.js";import"./three-Bv1OsDGp.js";const f=["menstrual","follicular","ovulatory","luteal"];let p=null,y=!1,o=null;async function M(){if(y){await m(),$();return}y=!0,S(),await m(),$()}function S(){const e=document.getElementById("partner-learn-content");e&&(e.innerHTML=`
    <div class="partner-forecast-root">
      <div class="partner-forecast-header">
        <div class="partner-forecast-label">Learn her cycle</div>
        <div class="partner-forecast-day" id="partner-learn-day">â€”</div>
      </div>
  ...
  `)}async function m(){try{const e=await x();if(!(e!=null&&e.access_token)){p=null;return}const t=await fetch(w("/api/partner/today"),{headers:{Authorization:`Bearer ${e.access_token}`}});if(!t.ok){p=null;return}p=await t.json()}catch(e){console.warn("[partnerLearn] state fetch failed:",e.message),p=null}}function $(){if(!p){H();return}const{cycleDay:e,phase:t,cycleLength:s,hasPeriodHistory:r,womanName:n}=p,a=b(s||28);o||(o=f.includes(t)?t:"menstrual");const i=document.getElementById("partner-learn-day");i&&(r?i.innerHTML=`
      ...
      <div
        ...
        data-phase="${a}"
        title="${u[a]} Â· Days ${i.start}-${i.end}"
      ></div>
      ...
      <div class="partner-phase-tile ${l?"is-current":""} ${d?"is-expanded":"is-collapsed"}" data-phase="${n}">
        <button class="partner-phase-tile-header" type="button" data-phase-toggle="${n}">
          ...
          <div class="partner-phase-tile-titlebox">
            ...
            <div class="partner-phase-tile-range">Days ${i.start}â€“${i.end}${l?" Â· she's here now":""}</div>
          </div>
          <div class="partner-phase-tile-chevron">${d?"â–¾":"â–¸"}</div>
        </button>
    ...
    `}).join(""),r.querySelectorAll("[data-phase-toggle]").forEach(n=>{n.addEventListener("click",()=>{const a=n.dataset.phaseToggle;o=o===a?null:a,o||(o=e),h(e,t,s)})}))}function A(e,t,s,r,n){if(!s||s<1)return"Once she logs her cycle, you'll see exactly where this phase falls relative to today.";if(e===t){const l=s-r.start+1,d=r.end-s;return d<0?"This phase is wrapping up.":d===0?`Day ${l} of ${r.days} â€” this phase ends today.`:`Day ${l} of ${r.days} â€” about ${d} more day${d===1?"":"s"} in this phase.`}if(r.start>s){const l=r.start-s;return`Begins in ${l} day${l===1?"":"s"}.`}const a=s-r.end;if(a===0)return"Just ended.";if(a<0)return"";const i=n.total-s+r.start;return`Last phase. Ended ${a} day${a===1?"":"s"} ago. Returns in ${i} days.`}function H(){const e=document.getElementById("partner-learn-day");e&&(e.textContent="Cycle data unavailable",e.style.fontSize="15px");const t=b(28);o||(o="menstrual"),E(t,null,null,!1);const s=document.getElementById("partner-learn-marker");s&&(s.style.display="none"),h(null,t,null)}function c(e){return String(e||"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}export{M as initPartnerLearnTab};
